...
<br>
<footer class="footer mt-auto py-3 text-muted ${UI.footer_style_class}" style="${UI.fixed_footer ?'position: fixed; ': ''}left: 0; bottom: 0; width: 100%; color: white; z-index: 9999;"> <div class="container" style="width: auto; padding: 0 10px;"> <p class="float-end"> <a href="#">Trở về đầu trang</a> </p> ${UI.credit ? '<p>Mod <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://bankhonggioi.com" target="_blank">Bạn không giỏi</a></p>' : ''} <p><a href="${UI.website_link}" target="_blank"> BKoG</a> | <a href="${UI.fb_group}" target="_blank"> FB Group </a> | <a href="${UI.mess_link}" target="_blank"> Messenger </a> | <a href="${UI.ytb_link}" target="_blank"> Youtube</a> |<a href="https://bankhonggioi.top/udemycoupon" target="_blank"> Udemy coupon</a> | <a href="${UI.link_donate}" target="_blank"> Donate với Momo</a> | <a> ${UI.dung_luong} </a></p> </div> </footer>
`;
  $('body').html(html);
}
...
function title(path) {
  ...
  if (model.is_search_page)
      $('title').html(`${drive_name} - Kết quả tìm kiếm cho ${model.q} `);
  else
...
<ul class="navbar-nav me-auto mb-2 mb-lg-0">
  ...
  html += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">${UI.nav_link_3}</a><div class="dropdown-menu" aria-labelledby="navbarDropdown"><a class="dropdown-item"  href="/">> Trang chủ</a>`;
...
function requestListPath(path, params, resultCallback, authErrorCallback) {
  ...
  $.post(path, p, function(data, status) {
    ...
    $('#list').html(`<div class='alert alert-danger' role='alert'> Không thể lấy dữ liệu từ Server. </div></div></div>`);
});
}
...
<ol class="breadcrumb" id="folderne"><li class="breadcrumb-item"><a href="/">Trang chủ</a></li>`;
var navlink = '';
...
<div id="readme_md" style="display:none; padding: 20px 20px;"></div>
  ...
  requestListPath(path, {
      ...
      function(path) {
          $('#spinner').remove();
          var pass = prompt("Thư mục được bảo vệ, hãy nhập mật khẩu để truy cập", "");
...
function onSearchResultItemClick(a_ele) {
    ...
    $.post(`${searchhost}:id2path`, {
        ...
        if (data) {
            ...
            title = `Kết quả`;
            $('#SearchModelLabel').html(title);
            content = `<a class="btn btn-info" href="${ehrefurl}">Mở</a> <a class="btn btn-secondary" href="${ehrefurl}" target="_blank">Mở trong Tab mới</a>`;
            $('#modal-body-space').html(content);
            return;
        }
        $('#modal-body-space').html(content);
...
<div class="card-text text-center">
${UI.display_drive_link_rar ? '<a type="button" class="btn btn-info" href="https://drive.google.com/file/d/'+ xxx +'/view" id ="file_drive_link" target="_blank">Google Drive Link</a>': ''}
<div class="btn-group text-center">
  <a href="${url}" type="button" class="btn btn-primary">Direct Download</a>
  <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    <span class="sr-only"></span>
  </button>
  <div class="dropdown-menu">
    <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${decodename};end">Tải bằng 1DM (Android)</a>
    <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${decodename};end">Tải bằng 1DM Lite (Android)</a>
    <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${decodename};end">Tải bằng 1DM+ (Android)</a>
  </div>
</div>
...
<div class="${UI.file_view_alert_class}" id="file_details" role="alert">${obj.name}<br>${size}</div>
<div>
<pre <pre ${UI.second_domain_for_dl ? 'style="display:none;"': 'style="display:block;"'} class="line-numbers language-markup" data-src="plugins/line-numbers/index.html" data-start="-5" style="white-space: pre-wrap; counter-reset: linenumber -6;" data-src-status="loaded" tabindex="0"><code id="editor"></code></pre>
</div>
</div>
<div class="card-body">
<div class="input-group mb-4">
...
<track kind="captions" label="Vietnamese" src="${caption}.vi.vtt" srclang="vi" />
...
${UI.display_drive_link ? '<a type="button" class="btn btn-info" href="https://drive.google.com/file/d/'+ xxx +'/view" id ="file_drive_link" target="_blank">Google Drive Link</a>': ''}
<div class="btn-group text-center">
<a href="${url}" type="button" class="btn btn-primary">Direct Download</a>
<button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
  <span class="sr-only"></span>
...
<div class="dropdown-menu">
  <a class="dropdown-item" href="iina://weblink?url=${url}">IINA (MacOS)</a>
  ...
  <a class="dropdown-item" href="nplayer-${url}">nPlayer (Mobile)</a>
  <a class="dropdown-item" href="intent://${url_without_https}#Intent;type=video/any;package=is.xyz.mpv;scheme=https;end;">mpv-android (Mobile)</a>
  <a class="dropdown-item" href="mpv://${url_base64}">mpv x64</a>
  <a class="dropdown-item" href="intent:${url}#Intent;package=com.mxtech.videoplayer.ad;S.title=${decodename};end">MX Player (Mobile)</a>
  <a class="dropdown-item" href="intent:${url}#Intent;package=com.mxtech.videoplayer.pro;S.title=${decodename};end">MX Player Pro (Mobile)</a>
  <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${decodename};end">Tải bằng 1DM (Android)</a>
  <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${decodename};end">Tải bằng 1DM Lite (Android)</a>
  <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${decodename};end">Tải bằng 1DM+ (Android)</a>
</div>
